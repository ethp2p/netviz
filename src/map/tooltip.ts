...
function renderSparkline(
  ...
  return `<svg width="${w}" height="${h}" style="display:block;margin:3px 0">` +
    ...
    `<line x1="${crossX.toFixed(1)}" y1="0" x2="${crossX.toFixed(1)}" y2="${h}" stroke="${P.text.css}" stroke-width="1" opacity="0.5" stroke-dasharray="2,2"/>` +
    `</svg>`;
}
...
export function renderNodeTooltip(
  ...
  const countryLabel = countryVal ? ' <span style="color:' + P.text3.css + '">' + escapeHtml(String(countryVal)) + '</span>' : '';
  const stateName = states[ns.state]?.label ?? formatDefinitionName(states[ns.state]?.name ?? 'unknown');
  let html = '<b>' + escapeHtml(d.name) + '</b>' + countryLabel + ' <span style="color:' + P.text2.css + '">[' + escapeHtml(stateName) + ']</span>';
  ...
  if (d.chunksNeed > 0) {
    ...
    html += '<br>' + d.chunksHave + '/' + d.chunksNeed + ' chunks <span style="color:' + P.text2.css + '">(' + pct + '%)</span>';
  }
  ...
  getOverlayMetricGroups(metrics).forEach((group) => {
    ...
    overlayMetrics.push({
      label: group.label,
      color: group.color ? `rgb(${group.color[0]},${group.color[1]},${group.color[2]})` : P.text2.css,
      value,
    });
  });

  if (overlayMetrics.length > 0) {
    html += '<div style="display:flex;height:3px;width:140px;margin:3px 0 2px;overflow:hidden;background:' + P.borderSubtle.css + '">';
